				...
				- I will begin by just checking is AIE is set, since we don't get an interrupt it's either AF or AIE.
					...
					- Just as a last confirmation as im curious if the initial mirror/refresh overrides what I have or my config never reaches RAM. I removed everything, just set AIE = 1 then read it back right away and outputted that to the GPIO. It stayed low. Meaning that even with a AK from I2C during the refresh period nothing written is actually written to ram. As far as I can tell this seems undocumented.
						- ```
						  ...
						  ```
- Ok with that, I put all the code inside the while to emulate what it would be like to do it over and over when waking up from shutdown. It works great and an interrupt is thrown every new iteration. Now to work with shutdown and wakeup. From reading my previous notes it seems I jumped into it a bit too early. This time I will read the relevant sections (4.3 and 4.4) from the U0 reference manual to get a more thorough understanding.
	- Notes from reading:
		- Default mode = run mode
		- "The system clock, when exiting the Shutdown mode, is MSI at 4 MHz."
		- With the right configuration the MCU can debugged in low power modes (see section 38.2.6)
		- Pg 118, table 22 lists up to 5 wakeup pins, not including PB15 (the one i'm using). Pg 130, lists up to 6 wake up pins then specifies x can be [1,5] which is just 5. I'm not sure if these are intentional. Anyway PB15 shows up as wakeup in CubeMX and it's worked in the past.
		- To enter shutdown mode
			- a wait for interrupt or wait for event call is required. If there is a pending event or interrupt then it will not go into the shutdown mode
			- SLEEPDEEP bit is set on cortex-m0+ system control register
			- LPMS = "1XX" in PWR_CR1 register
			- WUFx bits are cleared in the PWR_SR1 register
		- A POR happens when exiting shutdown mode
		- To exit shutdown mode
			- If WFI was used, any interrupt acknowledged by the NVIC will wake the MCU up
			- Pin should be enabled for rising or falling edge wakeup
		- PWR_CR3 bit 6 controls if wakeup pin 7 enabled or disabled
		- PWR_CR4 bit 6 controls if wakeup pin 7 is triggered on rising or falling edge
		- PWR_SR1 bit 6 is set if wakeup happened because of wakeup pin 7
		- PWR_CR3, PWR_CR4, PWR_SR1 are not reset on wake from shutdown
	- I reconfigured the RTC project by removing the GPIO output pin and made RTC interrupt pin a wakeup pin. Addressing the aug 8 backlog of the PWR section in CubeMX, by pressing on the pin then selecting wakeup pin it automatically checks the box in the `PWR>WakeUp from standby and shutdown mode configuration` section so nothing different from before.
	- As before I initialize by setting fd, alarm time, alarm enable and now enable wakeup pin 7 when going low. Then set the time, handle the interrupt (in case we woke up), clear the power flag, wait so we are awake for some time and to have time to flash, then enter shutdown mode. It works perfectly now.